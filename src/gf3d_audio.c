...
void gf3d_audio_close();
void gf3d_sound_init(Uint32 max);
	...
	Uint8  enableOgg)
{
	...
	atexit(Mix_Quit);
	atexit(gf3d_audio_close);
	gf3d_sound_init(maxSounds);
...
void gf3d_sound_close()
{
	gf3d_sound_clear_all();
...
void gf3d_sound_init(Uint32 max)
{
	...
	atexit(gf3d_sound_close);
}

void gf3d_sound_delete(Sound* sound)
{
	if (!sound)return;
	if (sound->sound != NULL)
	{
...
void gf3d_sound_clear_all()
{
	...
	{
		gf3d_sound_delete(&sound_manager.sound_list[i]);// clean up the data
	}
}

Sound* gf3d_sound_new()
{
	int i;
	...
	for (i = 0; i < sound_manager.max_sounds; i++)
	{
		...
		{
			gf3d_sound_delete(&sound_manager.sound_list[i]);// clean up the old data
			sound_manager.sound_list[i].ref_count = 1;//set ref count
			return &sound_manager.sound_list[i];//return address of this array element
		}
	}
...
Sound* gf3d_sound_get_by_filename(char* filename)
{
	int i;
	for (i = 0; i < sound_manager.max_sounds; i++)
	{
...
{
	Sound* sound;
	sound = gf3d_sound_get_by_filename(filename);
	if (sound)
	{
		sound->ref_count++;
		return sound;
